...
int versaoso(char *b, size_t s) {//Função mostra na tela os dados da versão do sistema
  ...
  if(uname(&my_uname) == -1){
    ...
    char buffer[500];
    buffer[0] = '\x0';
    snprintf(buffer,500, "S.O.: %s<br>Nome personalizado: %s<br>Release: %s<br>Vers&atildeo: %s<br>Arquitetura: %s<br>",
       my_uname.sysname, my_uname.nodename, my_uname.release,my_uname.version,my_uname.machine);
    strncat(b, "<h4>Vers&atildeo do Sistema Operacional</h4>", s);
    strncat(b, buffer, s);
  }
	return 1;
}
